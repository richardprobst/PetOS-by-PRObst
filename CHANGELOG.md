...
- A rodada de homologacao operacional da Fase 5 no host publicado passou a ficar registrada como `GO COM RESSALVAS`: o centro administrativo responde sem erro e o admin legado navega corretamente, mas o ambiente ainda expõe `MIGRATION_PENDING` para as tabelas novas da fase ate executar `prisma migrate deploy`.
