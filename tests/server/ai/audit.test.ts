...
test('writeAiExecutionAuditLogs emits a completed audit entry for an allowed execution path', async () => {
  ...
  assert.equal(
    audit.entries[0]?.details.operational?.cost?.status,
    'ESTIMATED',
  )
  assert.equal(
    audit.entries[0]?.details.observability?.executionStatus,
    'COMPLETED',
  )
  assert.equal(
    audit.entries[0]?.details.multiUnitContext?.contextType,
    'LOCAL',
  )
  assert.equal(
    audit.entries[0]?.details.consent?.decisionStatus,
    'ALLOWED',
  )
  assert.equal(
    audit.entries[0]?.details.retention?.policyVersion,
    'PHASE3_B2_BASELINE',
  )
})
...
test('writeAiExecutionAuditLogs emits blocked and fallback audit entries for a blocked policy path', async () => {
  ...
  assert.equal(
    audit.entries[0]?.details.outcome?.error?.code,
    'DISABLED',
  )
  assert.equal(
    audit.entries[0]?.details.consent?.decisionStatus,
  ...
  assert.equal(
    audit.entries[1]?.details.operational?.fallback?.status,
    'NOT_ELIGIBLE',
  )
  assert.equal(
    audit.entries[1]?.details.operational?.fallback?.reasonCode,
    'BLOCKED_BY_POLICY',
  )
})
...
test('writeAiExecutionAuditLogs emits fallback and human decision entries for a failed execution with declared fallback', async () => {
  ...
  assert.equal(
    audit.entries[1]?.details.operational?.fallback?.nextStep,
    'REVIEW_FUTURE_FALLBACK',
  )
  assert.equal(
    audit.entries[2]?.details.multiUnitContext?.contextType,
    'GLOBAL_AUTHORIZED',
  )
  assert.equal(
    audit.entries[2]?.details.humanDecision?.decisionType,
    'CONFIRM_ASSISTIVE_REVIEW',
  )
  assert.equal(
    audit.entries[0]?.details.consent?.reasonCode,
    'CONSENT_GRANTED',
  )
})
